...
namespace SenseNet.ContentRepository.Storage
{
    public class TreeLock : IDisposable
    {
        ...
        public static TreeLock Acquire(params string[] paths)
        {
            ...
            var lockIds = paths.Select(p => DataStore.Enabled ? DataStore.AcquireTreeLock(p) : DataProvider.Current.AcquireTreeLock(p)).ToArray(); //DB:ok
            for (int i = 0; i < lockIds.Length; i++)
            {
                if (lockIds[i] == 0)
                {
                    if (DataStore.Enabled) DataStore.ReleaseTreeLock(lockIds); else DataProvider.Current.ReleaseTreeLock(lockIds); //DB:ok
        ...
        public static void AssertFree(params string[] paths)
        {
            ...
            {
                if (DataStore.Enabled ? DataStore.IsTreeLocked(path) : DataProvider.Current.IsTreeLocked(path)) //DB:ok
                {
        ...
        public void Dispose()
        {
            if(DataStore.Enabled) DataStore.ReleaseTreeLock(_lockIds); else DataProvider.Current.ReleaseTreeLock(_lockIds); //DB:ok
        ...
        {
            return DataStore.Enabled ? DataStore.LoadAllTreeLocks() : DataProvider.Current.LoadAllTreeLocks(); //DB:ok
        }
    }
}
