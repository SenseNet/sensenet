...
namespace SenseNet.ContentRepository.Storage.Data.MsSqlClient
{
    ...
    public class MsSqlDataContext2 : SnDataContext
    {
        private readonly string _connectionString;

        public MsSqlDataContext2(CancellationToken cancellationToken = default(CancellationToken)) : base(cancellationToken) { }
        public MsSqlDataContext2(string connectionString, CancellationToken cancellationToken = default(CancellationToken)) :
            base(cancellationToken)
        {
            //UNDONE:DB: TEST: not tested (packaging)
            _connectionString = connectionString;
        }
        public MsSqlDataContext2(ConnectionInfo connectionInfo, CancellationToken cancellationToken = default(CancellationToken)) :
            base(cancellationToken)
        {
            //UNDONE:DB: TEST: not tested (packaging)
            _connectionString = GetConnectionString(connectionInfo);
        ...
        public int ExecuteNonQuery(string script, Action<SqlCommand> setParams = null)
        {
            using (var op = SnTrace.Database.StartOperation(GetOperationMessage("ExecuteNonQuery", script)))
            {
                using (var cmd = CreateSqlCommand())
                {
                    SqlTransaction transaction = null;
        ...
        public object ExecuteScalar(string script, Action<SqlCommand> setParams = null)
        {
            using (var op = SnTrace.Database.StartOperation(GetOperationMessage("ExecuteScalar", script)))
            {
                using (var cmd = CreateSqlCommand())
                {
                    SqlTransaction transaction = null;
        ...
        public T ExecuteReader<T>(string script, Func<SqlDataReader, CancellationToken, T> callback)
        {
            return ExecuteReader(script, null, callback);
        }
        public T ExecuteReader<T>(string script, Action<SqlCommand> setParams,
            Func<SqlDataReader, CancellationToken, T> callback)
        {
            using (var op = SnTrace.Database.StartOperation(GetOperationMessage("ExecuteReader", script)))
            {
                try
                {
                    using (var cmd = CreateSqlCommand())
                    {
                        ...
                        {
                            cancellationToken.ThrowIfCancellationRequested();
                            var result = callback(reader, cancellationToken);
                            op.Successful = true;
                            return result;
                        }
                    }
