...
{
    /// <summary> 
    /// This is a PostgreSQL implementation of the <see cref="IPackagingDataProvider"/> interface.
    /// It requires the main data provider to be a <see cref="RelationalDataProviderBase"/>.
    /// </summary>
    public class PgSqlPackagingDataProvider : IPackagingDataProvider
    {
        private readonly RelationalDataProviderBase _mainProvider;

        public PgSqlPackagingDataProvider(DataProvider mainProvider)
        {
        ...
        private static readonly string InstalledComponentsScript = $@"-- PgSqlPackagingDataProvider.LoadInstalledComponents
...
";
        ...
        public async STT.Task<IEnumerable<ComponentInfo>> LoadInstalledComponentsAsync(CancellationToken cancellationToken)
        {
            ...
            using var op = SnTrace.Database.StartOperation("PgSqlPackagingDataProvider: LoadInstalledComponents()");
        ...
        private static readonly string IncompleteComponentsScript = $@"-- PgSqlPackagingDataProvider.LoadIncompleteComponents
...
";
        ...
        public async STT.Task<IEnumerable<ComponentInfo>> LoadIncompleteComponentsAsync(CancellationToken cancellationToken)
        {
            ...
            using var op = SnTrace.Database.StartOperation("PgSqlPackagingDataProvider: LoadIncompleteComponents()");
            using var ctx = _mainProvider.CreateDataContext(cancellationToken);
        ...
        public async STT.Task<IEnumerable<Package>> LoadInstalledPackagesAsync(CancellationToken cancellationToken)
        {
            ...
            using var op = SnTrace.Database.StartOperation("PgSqlPackagingDataProvider: LoadInstalledPackages()");
            using var ctx = _mainProvider.CreateDataContext(cancellationToken);
    ...
    (""Description"", ""ComponentId"", ""PackageType"", ""ReleaseDate"", ""ExecutionDate"",
     ""ExecutionResult"", ""ExecutionError"", ""ComponentVersion"", ""Manifest"") VALUES
    (@Description, @ComponentId, @PackageType, @ReleaseDate, @ExecutionDate,
     @ExecutionResult, @ExecutionError, @ComponentVersion, @Manifest)
RETURNING ""Id""";
        #endregion
        public async STT.Task SavePackageAsync(Package package, CancellationToken cancellationToken)
        {
            using var op = SnTrace.Database.StartOperation("PgSqlPackagingDataProvider: " +
...
";
        ...
        {
            using var op = SnTrace.Database.StartOperation("PgSqlPackagingDataProvider: DeleteAllPackages()");
            using var ctx = _mainProvider.CreateDataContext(cancellationToken);
            await ctx.ExecuteNonQueryAsync(@"TRUNCATE TABLE ""Packages"" RESTART IDENTITY CASCADE").ConfigureAwait(false);
            op.Successful = true;
        }
        ...
        public Dictionary<string, string> GetContentPathsWhereTheyAreAllowedChildren(List<string> names)
        {
            ...
            var sql = $"-- GetContentPathsWhereTheyAreAllowedChildren: [{string.Join(", ", names)}]" +
                      Environment.NewLine;
            sql += @"SELECT n.""Path""::TEXT, t.""Value"" FROM ""LongTextProperties"" t
