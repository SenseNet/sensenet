...
namespace SenseNet.ContentRepository.Linq
{
    internal class CQVisitor : ExpressionVisitor
    {
        ...
        private Stack<SnQueryPredicate> _queries = new Stack<SnQueryPredicate>();
        ...
        internal SnQueryPredicate GetQuery(Type sourceCollectionItemType, ChildrenDefinition childrenDef)
        {
            ...
            var bmq = _queries.Peek() as BooleanMemberPredicate;
            if (bmq != null)
            {
                _queries.Pop();
        ...
        protected override Expression VisitMember(MemberExpression node)
        {
            ...
            if (memberExpr.Type == typeof(bool))
                _queries.Push(new BooleanMemberPredicate
                {
                ...
                case "GetType":
                    {
                        ...
                        if (member != null && member.Member == typeof(Content).GetProperty("ContentHandler"))
                            _queries.Push(new ContentHandlerGetTypePredicate());
                        else
                ...
                case "IsAssignableFrom":
                    {
                        ...
                        if (_queries.Count == 0)
                            throw new NotSupportedException("IsAssignableFrom method is not supported" + node);
                        var q = _queries.Pop() as ContentHandlerGetTypePredicate;
                        if (q == null)
                            throw new NotSupportedException("IsAssignableFrom method is not supported" + node);
        ...
        protected override Expression VisitUnary(UnaryExpression node)
        {
            ...
            if (unary.NodeType == ExpressionType.Convert)
            {
                if (unary.Type == typeof(bool))
                {
                    ...
                    _queries.Push(new BooleanMemberPredicate { FieldName = name, Value = true, FromVisitUnary = true });
                }
            }
            else if (unary.NodeType == ExpressionType.Not)
            {
                var predicate = _queries.Peek();
        ...
        private void CombineQueriesOnStack(int count)
        {
            ...
            {
                var predicate = _queries.Pop();
                var bmp = predicate as BooleanMemberPredicate;
                if (bmp != null)
                    predicate = CreateTermQuery(bmp.FieldName, bmp.Value);
        ...
        private SnQueryPredicate CreateTypeIsQuery(Type targetType)
        {
        ...
        private bool TopLevelQueryIsTypeQuery()
        {
            ...
            var topLevel = _queries.Peek() as TextPredicate;
            if (topLevel == null)
                return false;

            return topLevel.FieldName == IndexFieldName.TypeIs || topLevel.FieldName == IndexFieldName.Type;
        }
        private bool RemoveDuplicatedTopLevelBooleanMemberPredicate(SnQueryPredicate predicate)
        {
            ...
            var topBmq = _queries.Peek() as BooleanMemberPredicate;
            if (topBmq == null)
                return false;
        ...
        private void BuildFieldExpr(BinaryExpression node)
        {
            ...
            // creating product
            SnQueryPredicate predicate = null;
            switch (node.NodeType)
            {
                case ExpressionType.Equal:               // term
                    if (fieldDatatype == typeof(bool))
                        ...
                        predicate = CreateTermQuery(fieldName, queryFieldValue);
                    break;
                case ExpressionType.NotEqual:            // -term
                    if (fieldDatatype == typeof(bool))
                        predicate = new BooleanMemberPredicate { FieldName = fieldName, Value = !(bool)value };
                    else
                        predicate = new LogicalPredicate(new List<LogicalClause> { new LogicalClause(CreateTermQuery(fieldName, queryFieldValue), Occurence.MustNot) });
                    break;
                case ExpressionType.GreaterThan:         // field:{value TO ]
                    predicate = CreateRangeQuery(fieldName, queryFieldValue, null, false, true);
                    break;
                case ExpressionType.GreaterThanOrEqual:  // field:[value TO ]
                    predicate = CreateRangeQuery(fieldName, queryFieldValue, null, true, true);
                    break;
                case ExpressionType.LessThan:            // field:[ to value}
                    predicate = CreateRangeQuery(fieldName, null, queryFieldValue, true, false);
                    break;
                case ExpressionType.LessThanOrEqual:     // field:[ to value]
                    predicate = CreateRangeQuery(fieldName, null, queryFieldValue, true, true);
                    break;
                default:
            ...
            if (node.Type == typeof(bool))
                if (RemoveDuplicatedTopLevelBooleanMemberPredicate(predicate))
                    _queries.Pop();
            _queries.Push(predicate);
        }
        /*???*/internal static TextPredicate CreateTermQuery(string fieldName, object value)
        {
            ...
            return new TextPredicate(fieldName, stringValue);
        }
        ...
        private void BuildBooleanExpr(BinaryExpression node)
        {
            ...
            switch (node.NodeType)
            {
                ...
                case ExpressionType.AndAlso: // &&
                    clauses.Add(new LogicalClause(_queries.Pop(), Occurence.Must));
                    clauses.Add(new LogicalClause(_queries.Pop(), Occurence.Must));
                    break;
                case ExpressionType.OrElse: // ||
                    clauses.Add(new LogicalClause(_queries.Pop(), Occurence.Should));
                    clauses.Add(new LogicalClause(_queries.Pop(), Occurence.Should));
                    break;
                default:
