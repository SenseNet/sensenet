...
namespace SenseNet.Search
{
    internal class SnIFilter
    {
        ...
        private static string ExtractTextFromIFilter(IFilter filter)
        {
            ...
            while (true)
            {
                ...
                if (result == IFilterReturnCode.FILTER_E_END_OF_CHUNKS)
                {
                    return sb.ToString();
                }
        ...
        private static string ExtractTextFromChunk(IFilter filter, STAT_CHUNK chunk)
        {
            var sb = new StringBuilder();

            var result = IFilterReturnCode.S_OK;
            while (result == IFilterReturnCode.S_OK)
            {
                uint uSizeBuffer = 16384;
                var buffer = new char[uSizeBuffer];

                result = filter.GetText(ref uSizeBuffer, buffer);

                if ((result == IFilterReturnCode.S_OK) || (result == IFilterReturnCode.FILTER_S_LAST_TEXT))
                {
                    if ((uSizeBuffer > 0) && (buffer.Length > 0))
                    {
                        sb.Append(buffer, 0, (int)uSizeBuffer);
                    }
                }

                if (result == IFilterReturnCode.FILTER_E_NO_TEXT)
                {
                    return string.Empty;
                }

                if ((result == IFilterReturnCode.FILTER_S_LAST_TEXT) || (result == IFilterReturnCode.FILTER_E_NO_MORE_TEXT))
                {
                    return sb.ToString();
                }
            }
    ...
    internal static class NativeMethods
    {
        ...
        public static extern IFilterReturnCode LoadIFilter(
            string pwcsPath,
