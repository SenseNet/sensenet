...
namespace SenseNet.ContentRepository.Tests
{
    ...
    public class AccessTokenTests : TestBase
    {
        ...
        {
            await AccessTokenTestAsync(async () =>
            {
        ...
        private async Tasks.Task AccessTokenTestAsync(Func<Tasks.Task> callback)
        {
            using (CreateSwindler())
                await callback();
        }
        private void AccessTokenTest(Action callback)
        {
            using (CreateSwindler())
                callback();
        }
        private IDisposable CreateSwindler()
        {
            var dp = new InMemoryDataProvider();
            dp.SetExtension(typeof(IAccessTokenDataProviderExtension), new InMemoryAccessTokenDataProvider());
            return new Swindler<DataProvider>(dp,
                () => Providers.Instance.DataProvider,
                value => Providers.Instance.DataProvider = dp);
        }
