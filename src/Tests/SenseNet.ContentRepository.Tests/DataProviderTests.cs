...
";
            await Test(async () =>
            {
                ...
                try
                {
                    ...
                    Assert.AreEqual($"{Repository.Root.Id},{root.Id}", ArrayToString((IEnumerable<object>)await TDP.GetPropertyValueAsync(node.VersionId, "Reference1")));
                    ...
                    Assert.AreEqual($"{root.Id}", ArrayToString((IEnumerable<object>)await TDP.GetPropertyValueAsync(node.VersionId, "Reference1")));
