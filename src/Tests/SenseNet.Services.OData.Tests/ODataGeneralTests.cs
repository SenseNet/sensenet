...
//            var origIds = Node.Load<Folder>("/Root/IMS/BuiltIn/Portal").Children.Select(f => f.Id);
...
//            var origIds = Node.Load<Group>("/Root/IMS/BuiltIn/Portal/Administrators").Members.Select(f => f.Id);
//            var Ids = entities.Select(e => e.Id);
...
//            result = ODataGET<ODataRaw>("/OData.svc/Root('IMS')/Id", "");

//            json = result.ToString().Replace("\r\n", "").Replace("\t", "").Replace(" ", "");
...
//            var Ids = entities.Select(e => e.Id);
//            var origIds = ContentQuery
//                .Query("+InFolder:/Root/System/Schema/ContentTypes/GenericContent .REVERSESORT:Name .SKIP:4 .TOP:3 .AUTOFILTERS:OFF")
//                .Nodes.Select(n => n.Id);
