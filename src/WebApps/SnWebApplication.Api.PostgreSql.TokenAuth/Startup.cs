...
namespace SnWebApplication.Api.PostgreSql.TokenAuth
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            // [sensenet]: Set options for PgSqlSecurityDataProvider
            services.AddOptions<SenseNet.ContentRepository.Storage.Data.PgSqlClient.Security.PgSqlSecurityDataOptions>()
            ...
            // [sensenet]: PostgreSQL connection string mapping
            // The default ConfigureConnectionStrings reads "SnCrMsSql" from configuration.
            // For PostgreSQL we read "SensenetRepository" (or fallback to "SnCrMsSql").
            services.PostConfigure<ConnectionStringOptions>(options =>
            {
                var pgConnStr = Configuration.GetConnectionString("SensenetRepository");
                if (!string.IsNullOrEmpty(pgConnStr))
                {
                    options.Repository = pgConnStr;
                    options.Security ??= pgConnStr;
            ...
            services
                ...
                .AddPgSqlSecurityDataProvider()
                .AddSenseNetPgSqlProviders(configureInstallation: installOptions =>
                {
                    Configuration.Bind("sensenet:install:postgres", installOptions);
                })
                .AddSenseNetOData()
                .AddSenseNetWebHooks()
                .AddSenseNetWopi()
