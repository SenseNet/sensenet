...
# ============================================================
#  reset-postgres.sh
#  Stops, resets, and restarts the sensenet PostgreSQL stack.
#
#  Usage:
#    ./reset-postgres.sh              # full reset + rebuild
#    ./reset-postgres.sh --no-build   # full reset, no rebuild
#    ./reset-postgres.sh --help
# ============================================================
...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      ...
      echo ""
      echo "Stops all containers, drops the PostgreSQL database,"
...
echo -e "${BOLD}"
echo "╔═══════════════════════════════════════════════╗"
echo "║   sensenet PostgreSQL — Full Reset & Restart  ║"
echo "╚═══════════════════════════════════════════════╝"
echo -e "${NC}"
...
step "Starting PostgreSQL and running DB reset..."
...
if [ $ELAPSED -ge $MAX_WAIT ]; then
  echo ""
  warn "Timed out after ${MAX_WAIT}s. Check logs: docker compose -f docker-compose.postgres.yml logs -f snapp"
fi
