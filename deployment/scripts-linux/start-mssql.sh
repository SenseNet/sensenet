...
# ============================================================
#  start-mssql.sh
#  Starts the sensenet MSSQL stack.
#
#  Usage:
#    ./start-mssql.sh              # start (rebuild if needed)
#    ./start-mssql.sh --no-build   # start without rebuild
#    ./start-mssql.sh --build      # force rebuild snapp image
#    ./start-mssql.sh --help
# ============================================================
...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      ...
      echo ""
      echo "Starts the sensenet MSSQL stack."
      echo ""
      echo "Options:"
...
echo -e "${BOLD}"
echo "╔═══════════════════════════════════════════════╗"
echo "║   sensenet MSSQL — Start                      ║"
echo "╚═══════════════════════════════════════════════╝"
echo -e "${NC}"

# ── Start ────────────────────────────────────────────────────
step "Starting the MSSQL stack..."
...
if [ $ELAPSED -ge $MAX_WAIT ]; then
  echo ""
  echo -e "  ${CYAN}⚠ Timed out. Check logs: docker compose -f docker-compose.mssql.yml logs -f snapp${NC}"
fi
...
echo ""
echo -e "  ${CYAN}API Key:${NC}     docker compose -f docker-compose.mssql.yml run --rm apikey"
